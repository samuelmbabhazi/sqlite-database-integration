...
// ---- Partial-compilation options (method-size capping with runtime fallback) ----
$cap         = null;   // max method lines; bigger rules become runtime stubs.
$stub_single = false;  // stub all single-candidate rules regardless of size.
$only_rids   = null;   // if set, compile ONLY these rule ids; stub the rest.
foreach ( $argv as $arg ) {
	if ( preg_match( '/^--cap=(\d+)$/', $arg, $m ) ) {
		$cap = (int) $m[1];
	}
	if ( '--stub-single-candidate' === $arg ) {
		$stub_single = true;
	}
	if ( preg_match( '/^--only-rids-file=(.+)$/', $arg, $m ) ) {
		$only_rids = array_fill_keys(
			array_map( 'intval', file( $m[1], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES ) ),
			true
		);
	}
}
...
// Emit each kept rule's compiled body once, then decide which to replace with
// a 3-line stub that delegates to the runtime parse_recursive().
$compiled = array();
foreach ( $kept as $rid => $_ ) {
	$compiled[ $rid ] = $emit_method( $rid );
}
$stub = array();
foreach ( $kept as $rid => $_ ) {
	$lines   = substr_count( $compiled[ $rid ], "\n" );
	$is_stub = false;
	if ( null !== $cap && $lines > $cap ) {
		$is_stub = true;
	}
	if ( $stub_single && isset( $grammar->single_candidate_rules[ $rid ] ) ) {
		$is_stub = true;
	}
	if ( null !== $only_rids && ! isset( $only_rids[ $rid ] ) ) {
		$is_stub = true;
	}
	// Always keep the start rule compiled so parsing begins in compiled code.
	if ( $rid === $query_rid ) {
		$is_stub = false;
	}
	if ( $is_stub ) {
...
echo "class WP_MySQL_Compiled_Parser extends WP_Parser {\n";
...
foreach ( $kept as $rid => $_ ) {
	if ( isset( $stub[ $rid ] ) ) {
		echo "\tprivate function " . $method_name( $rid ) . "() {\n";
...
$compiled_count = count( $kept ) - count( $stub );
fwrite( STDERR, sprintf( "compiled=%d stubbed=%d total=%d\n", $compiled_count, count( $stub ), count( $kept ) ) );
