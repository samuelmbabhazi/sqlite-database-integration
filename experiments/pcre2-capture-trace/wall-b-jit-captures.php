<?php
/**
 * Wall (b): does adding ~6 numbered captures around (?&rule) call sites collapse
 * JIT validate throughput? Build the real grammar pattern with 0 vs 6 captures,
 * measure preg_match QPS over 30K pre-encoded queries (warm JIT, best-of-N).
 */
...
$build=function($ncap)use($compiled,$idx_to_rule,$rule_to_idx,$start_rid){
  $wrapped=0; $define='';
  foreach($idx_to_rule as $rid){ $body=$compiled[$rid];
    $body=preg_replace_callback('/RREF(\d+)RREF/',function($m)use($rule_to_idx,&$wrapped,$ncap){
      $ref='(?&r'.$rule_to_idx[(int)$m[1]].')'; if($wrapped<$ncap){$wrapped++;return '('.$ref.')';} return $ref; },$body);
    $define.="(?<r{$rule_to_idx[$rid]}>{$body})"; }
  return '/(?(DEFINE)'.$define.')\A(?&r'.$rule_to_idx[$start_rid].')\z/u'; };

// Load + encode 30K queries.
...
$bench=function($pat)use($enc,$n){
  $run=function()use($pat,$enc){ $s=microtime(true); foreach($enc as $e)@preg_match($pat,$e); return microtime(true)-$s; };
  for($i=0;$i<2;$i++)$run(); $qs=array(); for($r=0;$r<7;$r++)$qs[]=$n/$run(); sort($qs); return $qs[count($qs)-1]; };

foreach(array(0,6) as $ncap){
  $pat=$build($ncap);
  $ok=@preg_match($pat,"\xff"); // warm compile
  $qps=$bench($pat);
  printf("captures=%d  pattern=%s bytes  validate=%d QPS\n",$ncap,number_format(strlen($pat)),$qps);
}
